...
const modelConfigDescriptions = {
  ...
  maintainer:
    "Maintainer of the model. EvalFlow managed models can be cloned, user managed models can be edited and deleted. To supersede a EvalFlow managed model, set the custom model name to the EvalFlow model name.",
...
export default function ModelsTable({ projectId }: { projectId: string }) {
  ...
  const columns: LangfuseColumnDef<ModelTableRow>[] = [
    ...
    },
    {
      ...
      size: 60,
      cell: ({ row }) => {
        const isLangfuse = row.original.maintainer === "EvalFlow";
        return (
          <div className="flex justify-center">
            <Tooltip>
              ...
              <TooltipContent>
                {isLangfuse ? "EvalFlow maintained" : "User maintained"}
              </TooltipContent>
            </Tooltip>
          </div>
        );
    ...
    }),
    {
      ...
      size: 120,
      cell: ({ row }) => {
        return row.original.maintainer !== "EvalFlow" ? (
          <div
  ...
  const convertToTableRow = (model: GetModelResult): ModelTableRow => {
    ...
    return {
      modelId: model.id,
      maintainer: model.projectId ? "User" : "EvalFlow",
