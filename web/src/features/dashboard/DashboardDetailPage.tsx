...
function DashboardDetailView({ readPath }: { readPath: ResolvedReadPath }) {
  ...
  // EvalFlow-managed dashboards keep full edit affordances; edit attempts
  ...
  // owner is known ("do the EvalFlow-maintained templates get used?"). The
  ...
  // Add a EvalFlow Home card as a preset placement (no widget row involved).
    ...
    async (source: "dashboard_menu", anchor?: DashboardPlacement) => {
      ...
      if (parsed.status === "not-widget") {
        ...
        showErrorToast(
          ...
          "The clipboard does not contain a EvalFlow widget JSON. Copy one via a widget's ⋯ menu first.",
          "WARNING",
        );
        return;
      }
  ...
  // the clipboard actually holds a EvalFlow widget payload and the paste is
      ...
      try {
        ...
        if (imported.skippedPresetCount > 0) {
          showErrorToast(
            ...
            `${imported.skippedPresetCount} preset card(s) in the file are not available in this EvalFlow version.`,
            "WARNING",
          );
        }
      } catch (e) {
  ...
  const handleDroppedFile = useCallback(
    async (file: File) => {
      ...
      if (widgetResult.status === "not-widget") {
        ...
        showErrorToast(
          "Unsupported file",
          "Only EvalFlow dashboard or widget JSON files can be dropped here.",
          "WARNING",
        );
        return;
      }
                      ...
                      <div className="bg-background/80 pointer-events-none fixed inset-0 flex items-center justify-center backdrop-blur-xs">
                        <div className="border-primary bg-background rounded-lg border-2 border-dashed px-8 py-6 text-center shadow-lg">
                          ...
                          <p className="text-muted-foreground text-sm">
                            EvalFlow dashboard or widget JSON
                          </p>
                        </div>
                      </div>
                    </Layer>
