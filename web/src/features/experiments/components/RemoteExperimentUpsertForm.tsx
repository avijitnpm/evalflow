    ...
    for (const [index, header] of data.headers.entries()) {
      ...
      if (WebhookProtectedHeaders.includes(name.toLowerCase())) {
        form.setError(`headers.${index}.name`, {
          message: `"${name}" is set by EvalFlow and cannot be overridden`,
        });
        return;
      }
      requestHeaders[name] = {
    ...
    return (
      <>
        <DialogHeader>
          ...
          <DialogDescription>
            EvalFlow signs every remote experiment request with this secret via
            the <code>x-langfuse-signature</code> header. Store it in your
            service to verify that requests come from EvalFlow. It can only be
            viewed once.
          </DialogDescription>
        </DialogHeader>
        <DialogBody>
    ...
    <>
      <DialogHeader>
        ...
        <DialogDescription>
          ...
          which can run against the dataset and post results to EvalFlow.
        </DialogDescription>
      </DialogHeader>
                    ...
                    <FormDescription>
                      {field.value
                        ? existingRemoteExperiment?.displaySecretKey
                          ? "Requests include an x-langfuse-signature header so your service can verify they come from EvalFlow."
