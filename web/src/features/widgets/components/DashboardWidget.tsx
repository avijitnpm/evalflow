...
export function DashboardWidget({
  ...
  schedulerId?: string;
  /**
   * Present on EvalFlow-managed (read-only) dashboards: edit affordances stay
  ...
  // EvalFlow-managed dashboard, but the user could edit a clone: show the
        ...
        >
          ...
          {dashboardOwner === "PROJECT" && widget.data.owner === "LANGFUSE" && (
            <Badge
              variant="secondary"
              className="shrink-0"
              title="Maintained by EvalFlow — editing creates your own copy"
            >
              EvalFlow
            </Badge>
          )}
