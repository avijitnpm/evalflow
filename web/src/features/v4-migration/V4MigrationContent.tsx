...
// language-agnostic guidance: the upgrade applies to every EvalFlow SDK, not
...
// the steps as they apply to EvalFlow Cloud projects.
...
// Renders only while ingestion-API traffic without a EvalFlow SDK header is
...
export function V4MigrationCustomInstrumentationSection({
  ...
  return (
    ...
    >
      <p className="text-muted-foreground text-sm leading-relaxed">
        Data is arriving through the ingestion API without a EvalFlow SDK
                    ...
                    <ul className="mt-2 flex flex-col gap-2">
                      {callers.map((caller, index) => {
                        ...
                        const callerName = caller.isOther
                          ? "Unknown callers"
                          : caller.sdkName
                            ? `EvalFlow ${caller.sdkName === "python" ? "Python" : "JavaScript"} SDK${caller.sdkVersion ? ` ${caller.sdkVersion}` : ""}`
                            : codingAgent
                              ? codingAgent
 ...
 *
 * `V4_MIGRATION_DEADLINE` is a EvalFlow Cloud commitment. On a self-hosted
  ...
  return (
    <>
      ...
      <div className="text-muted-foreground flex flex-col gap-2 text-sm leading-relaxed">
        <p>
          EvalFlow v4 is live: a re-architecture of our data model and database
